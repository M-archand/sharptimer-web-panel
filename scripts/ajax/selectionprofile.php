<?php
    ...
    if($id === "%"){
        ...
        $sql = "SELECT `SteamID`, `PlayerName`, `FormattedTime`, `MapName`, `TimesFinished`, `TimerTicks`, RANK() OVER(ORDER BY `TimerTicks` ASC) AS 'Ranking' FROM PlayerRecords WHERE MapName LIKE '{$id}' ORDER BY `TimerTicks` ASC";
    }
    else{
        $sql = "SELECT `SteamID`, `PlayerName`, `FormattedTime`, `MapName`, `TimesFinished`, `TimerTicks`, RANK() OVER(ORDER BY `TimerTicks` ASC) AS 'Ranking' FROM PlayerRecords WHERE MapName LIKE '{$id}'";
    }
